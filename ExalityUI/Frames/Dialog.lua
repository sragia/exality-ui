...
---@class EXUIDialogFrame
local dialog = EXUI:GetModule('dialog-frame')
...
local function ConfigureFrame(f)
    ...
    f.fadeIn = EXUI.utils.animation.fade(f, 0.2, 0, 1)
    f.fadeOut = EXUI.utils.animation.fade(f, 0.2, 1, 0)
    f.fadeOut:SetScript('OnFinished', function() f:Hide() end)
    EXUI.utils.animation.diveIn(f, 0.2, 0, 20, 'IN', f.fadeIn)
    EXUI.utils.animation.diveIn(f, 0.2, 0, -20, 'OUT', f.fadeOut)

    local background = f:CreateTexture(nil, 'BACKGROUND')
    background:SetTexture(EXUI.const.textures.frame.bg)
