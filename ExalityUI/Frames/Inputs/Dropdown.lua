...
---@class EXUIDropdownInput
local dropdown = EXUI:GetModule('dropdown')
...
local function CreateOption(f, frameOptions)
    ...
    if (not option.valueDisplay) then
        ...
        valueDisplay:SetFont(EXUI.const.fonts.DEFAULT, 10, 'OUTLINE')
        valueDisplay:SetPoint('LEFT', 10, 0)
        valueDisplay:SetWidth(0)

        option.SetOption = function(self, value, label)
            if (f.optionData and f.optionData.isFontDropdown) then
...
local function PopulateOptions(f, options, frameOptions, selectedValue)
    ...
    local previous
    local count = 0
    for value, label in EXUI.utils.spairs(options, function(t, a, b) return t[a] < t[b] end) do
        count = count + 1
...
local function ConfigureFrame(f, options)
    ...
    if (not f.hoverContainer) then
        ...
        f.animDur = 0.15
        f.onHover = EXUI.utils.animation.fade(hoverContainer, f.animDur, 0.1, 1)
        f.onHoverLeave = EXUI.utils.animation.fade(hoverContainer, f.animDur, 1, 0.1)
        hoverBorder:SetVertexColor(0.9, 0.9, 0.9, 1)
    end

    if (not f.label) then
        local textFrame = f:CreateFontString(nil, 'OVERLAY')
        textFrame:SetFont(EXUI.const.fonts.DEFAULT, 10, 'OUTLINE')
    ...
    if (not f.optionContainer) then
        ...
        optionContainerBg:SetTexture(EXUI.const.textures.frame.bg)
