...
---@class EXUIKeybind
local keybind = EXUI:GetModule('keybind-frame')
...
local SetupKeyListening = function(f)
    ...
    local function onKeyDown(self, key)
        ...
        if (RESERVED_KEYS[key]) then
            EXUI.utils.printOut('Reserved key ' .. key .. ' pressed')
            return
        end
...
local SetupFrame = function(f)
    ...
    keybind:SetFont(EXUI.const.fonts.DEFAULT, 13, 'OUTLINE')
