...
local window = EXUI:GetModule('window-frame')

---@class EXUIWindowManager
local windowManager = EXUI:GetModule('window-manager')
...
local configure = function(frame)
    ...
    frame.fadeIn = EXUI.utils.animation.fade(frame, 0.2, 0, 1)
    frame.fadeOut = EXUI.utils.animation.fade(frame, 0.2, 1, 0)
    frame.fadeOut:SetScript('OnFinished', function() frame:Hide() end)
    EXUI.utils.animation.diveIn(frame, 0.2, 0, 20, 'IN', frame.fadeIn)
    EXUI.utils.animation.diveIn(frame, 0.2, 0, -20, 'OUT', frame.fadeOut)
    ...
    if (not frame.logo) then
        ...
        texture:SetTexture(EXUI.const.textures.logo)
        ...
        version:SetFont(EXUI.const.fonts.DEFAULT, 10, 'OUTLINE')
    ...
    if (not frame.resizeBtn) then
        ...
        resizeBtn:SetNormalTexture(EXUI.const.textures.frame.resizeBtn)
        resizeBtn:SetHighlightTexture(EXUI.const.textures.frame.resizeBtnHighlight)
    ...
    if (not frame.timer) then
        ...
        timerBg:SetTexture(EXUI.const.textures.frame.statusBar)
        ...
        frame.timer:SetStatusBarTexture(EXUI.const.textures.frame.statusBar)
    ...
    frame.title = title
    title:SetFont(EXUI.const.fonts.DEFAULT, 10, 'OUTLINE')
    title:SetTextColor(1, 1, 1)
    title:SetPoint('CENTER', frame, 'TOP')
    title:SetText('ExalityUI')
