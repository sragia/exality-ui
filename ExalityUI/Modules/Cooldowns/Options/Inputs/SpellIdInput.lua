...
---@class EXUICooldownsSpellIdInput
local spellIdInput = EXUI:GetModule('cooldowns-spell-id-input')
...
local function getProvider()
    return EXUI:GetModule('cooldowns-spell-index')
end
...
local function updateDisplayValue(frame, entry)
    if entry then
        ...
        frame.selectedName:SetText(string.format('%s (%d)', entry.name or tostring(entry.spellID), entry.spellID))
    else
        setRowIcon(frame.selectedIcon, nil)
        frame.selectedName:SetText('No spell selected')
    end
end

local function setSelected(frame, spellID, triggerChange)
    frame.selectedID = spellID and tonumber(spellID) or nil
...
local function resolveAndSet(frame, text)
    ...
    local spellID = provider:ResolveInput(text)
    if spellID then
        provider:RegisterSpellID(spellID)
        setSelected(frame, spellID, true)
    end
end
...
local function runAutocomplete(frame)
    ...
    local entries = provider:BuildSearchMenuEntries(results, function(spellID)
        setSelected(frame, spellID, true)
        hideAutocomplete(frame)
    end)
...
local function configureFrame(frame)
    ...
    frame.SetOptionData = function(self, option)
        ...
        self.sectionLabel:SetText(option.label or 'Spell')
...
spellIdInput.Create = function(self)
