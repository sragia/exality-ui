...
function containerOptions:GetOptions(displayID)
    ...
    for _, unitKey in ipairs(defaults.UNIT_ORDER) do
        local label = defaults.UNIT_OPTIONS[unitKey]
        table.insert(fields, {
            type = 'checkbox',
            label = label,
            name = 'unit_' .. unitKey,
            width = 33,
            currentValue = function()
                local units = ufAuras:GetDisplayValue(displayID, 'units') or {}
                return units[unitKey] == true
            end,
            onChange = function(value)
                local display = ufAuras:GetDisplay(displayID)
                if not display then return end
                display.units = display.units or {}
                display.units[unitKey] = value and true or nil
                ufAuras:UpdateDisplay(displayID, display)
                refreshDisplayAndList(displayID)
            end,
        })
    end
    ...
    append(fields, {
        { type = 'title', label = 'Position on Unit Frame', width = 100 },
        {
            type = 'anchor-point',
            label = 'Aura Anchor',
            name = 'anchorPoint',
